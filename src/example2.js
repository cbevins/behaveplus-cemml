 ...
 *
 * It demonstrates a batch run where input values are case-by-case samples
 * taken from various locations or at various times.
 * The input samples arrive from a stream (file or pipe), are processed in turn,
...
const fileName = 'example2.results.txt'
...
for(let i=0; i<samples; i++) {
  const sample = randomInput()
  dag.runInputs([
    ['site.moisture.live.category', sample.moisLive],
    ['surface.primary.fuel.model.catalogKey', sample.fuel],
    ['site.moisture.dead.category', sample.moisDead],
    ['site.canopy.crown.baseHeight', sample.canopyBaseHt],
    ['site.canopy.crown.totalHeight', sample.canopyTotalHt],
    ['site.canopy.cover', sample.canopyCover],
    ['site.slope.steepness.ratio', sample.slopeSteepness],
    ['site.wind.speed.at20ft', sample.windAt20Ft],
    ['site.canopy.fuel.bulkDensity', sample.canopyBulk], // only used if selecting crown fire outputs
    ['site.canopy.fuel.foliar.moistureContent', sample.canopyFoliarMoist],  // only used if selecting crown fire outputs
    ['site.fire.time.sinceIgnition', sample.timeSinceIgnition], // only used if selecting fire area or perimeter outputs
  ])
}
...
let rps = (samples / (0.001 * elapsed)).toFixed(0)
console.log(`\nThere were ${samples} runs requiring ${elapsed} milliseconds (${rps} runs/s)`)
